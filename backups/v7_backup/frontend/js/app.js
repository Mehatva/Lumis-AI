...
async function loadBusinesses() {
  ...
  if (!businesses || businesses.length === 0) {
    console.log("No business found, showing onboarding Step 1 on Dashboard.");
    document.body.classList.add("dashboard-locked");
    const overlay = document.getElementById("onboarding-overlay");
    if (overlay) overlay.classList.remove("hidden");
    App.onboardNextStep(1); // Show Brand Identity step
    Toast.show("Welcome! Let's set up your first business to continue.", "info");
  } else {
    State.businesses = businesses;
    const b = businesses[0];
    State.currentBusiness = b;
    
    // GATING: If plan is still 'trial', show onboarding overlay and lock UI
    if (!b.plan || b.plan === "trial") {
      document.body.classList.add("dashboard-locked");
      const overlay = document.getElementById("onboarding-overlay");
      if (overlay) overlay.classList.remove("hidden");
      // Direct them to step 2 (Plan Selection)
      App.onboardNextStep(2);
...
const App = {
  ...
  onboardNextStep(step) {
    ...
    // Toggle wide mode for Phase 2 (Plans) to allow the 3-tier grid to breathe
    const card = document.getElementById("onboarding-card");
    if (card) {
      if (step === 2) card.classList.add("pricing-view");
      else card.classList.remove("pricing-view");
    }
  },
  ...
  async onboardCreateBusiness(e) {
    ...
    if (result && !result.error) {
      ...
      this.onboardNextStep(2); // Move to Plans
      Toast.show("Business profile established. Proceeding to plans...", "success");
    } else {
      const msg = result?.error || "Failed to create business. Please try again.";
